...
def im2latex_cnn(X, num_feats, bn):
    ...
    X = tf.nn.relu(tflib.ops.conv2d('conv3', X, 3, 1, num_feats*2, num_feats*4,  pad = 'SAME', bias=False))
    ...
    X = tf.nn.relu(tflib.ops.conv2d('conv5', X, 3, 1, num_feats*4, num_feats*8,  pad = 'SAME', bias=False))
    X = tflib.ops.max_pool('pool5', X, k=(2,1), s=(2,1))

    X = tf.nn.relu(tflib.ops.conv2d('conv6', X, 3, 1, num_feats*8, num_feats*8,  pad = 'SAME', bias=False))
